...
$Log: dvfmEvsUmlCheckNickname.c,v $
...
main(int argc,char *argv[ ]) 
{
	...
	{
		printf("use: %s <nickname> <valid character> <minimum_size> <maximum_size>\n", argv[0]);
    ...
    dvfmEvsUmlErrorCode = DvfmEvsUmlCheckNickname(argv[1], argv[2], dvfmEvsUmlMinimumSize, dvfmEvsUmlMaximumSize);
	if (dvfmEvsUmlErrorCode)
    {
        printf("%s\n", DvfmEvsUmlGetCliErrorMessage(dvfmEvsUmlErrorCode, dvfmEvsUmlEnglish));
        exit(DVFM_EVS_OK);
    }
    printf("The nickname is valid.\n");
    
	return DVFM_EVS_OK;
}
/* $RCSfile: dvfmEvsUmlCheckNickname.c,v $ */
